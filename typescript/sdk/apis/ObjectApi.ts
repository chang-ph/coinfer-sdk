...
export class ObjectApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param deletedKey 
     */
    public async deleteObject(objid: string, deletedKey?: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'objid' is not null or undefined
        if (objid === null || objid === undefined) {
            throw new RequiredError("ObjectApi", "deleteObject", "objid");
        }
     ...
     * @param deletedKey 
     */
    public async deleteObjects(objids?: Array<string>, deletedKey?: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;
...
export class ObjectApiResponseProcessor {
     ...
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async deleteObjectsWithHttpInfo(response: ResponseContext): Promise<HttpInfo<SuccRspSoftDeletedRsp >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: SuccRspSoftDeletedRsp = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
