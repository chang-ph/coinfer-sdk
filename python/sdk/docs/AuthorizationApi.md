...
# **update**
> SuccRspUnionUserInfoRspGetTokensRsp update(update_base)
...
with openapi_client.ApiClient(configuration) as api_client:
    # Create an instance of the API class
    api_instance = openapi_client.AuthorizationApi(api_client)
    update_base = openapi_client.UpdateBase() # UpdateBase | 

    try:
        # Update current user or a token
        api_response = api_instance.update(update_base)
        print("The response of AuthorizationApi->update:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling AuthorizationApi->update: %s\n" % e)
```
