...
class GetNotificationReq(BaseModel):
    """
    GetNotificationReq
    """ # noqa: E501
    ...
    status: StrictStr = Field(description="Filter the notification message by read status")
    __properties: ClassVar[List[str]] = ["page_no", "page_size", "status"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['all', 'read', 'unread']):
            raise ValueError("must be one of enum values ('all', 'read', 'unread')")
        return value
